 ...
 *
 */
abstract class BaseSearch(graph: Graph) : Alg(graph), Search {

    override fun search(vertex: Int) {
        println("Searching graph for vertex $vertex")
        run(vertex - 1)
        println()
        println("Search ended")
    }

    protected abstract fun run(x: Int)
